...
pf_server <- function(id, data, input, output) {
  ...
  output$pf_selects <- renderUI({
    ...
    if('site' %in% colnames(pf_dat())){
      bloop <- pf_dat() %>% distinct(site) %>% pull()
    }
    
    if('time_increment' %in% colnames(pf_dat())){
      t <- pf_dat() %>% distinct(time_increment) %>% pull()
    }else{t <- NULL}
    
    if('visit_type' %in% colnames(pf_dat())){
      vis <- pf_dat() %>% distinct(visit_type) %>% pull()
    }else{vis <- NULL}
    
    if('domain' %in% colnames(pf_dat())){
      d <- pf_dat() %>% distinct(domain) %>% pull()
    }else{d <- NULL}
  ...
  pf_graph <- reactive({
    ...
    if(grepl('Single Site', input$pf_type) & !is.null(input$pf_site_s)){
      pfdf <- pf_dat() %>% filter(site == input$pf_site_s)
    }else{
      pfdf <- pf_dat()
    }
    
    if(grepl('Longitudinal', input$pf_type) & !is.null(input$time)){
      pfdf <- pf_dat() %>% filter(time_increment == input$pf_time)
    }else{
      pfdf <- pf_dat()
    }
