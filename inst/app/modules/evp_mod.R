...
evp_server <- function(id, data, input, output) {
    ...
    output$evp_selects <- renderUI({
      ...
      if('site' %in% colnames(evp_dat())){
        bloop <- evp_dat() %>% distinct(site) %>% pull()
      }else(bloop <- evp_dat() %>% distinct(grp) %>% pull())
      
      if('time_increment' %in% colnames(evp_dat())){
        t <- evp_dat() %>% distinct(time_increment) %>% pull()
      }else{t <- NULL}
      ...
      base_type <- str_remove(input$evp_type, ' \\(.*')
